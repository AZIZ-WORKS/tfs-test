...
process.title = `NodeCG - ${pjson_1.pjson.version}`;
...
fetch("https://registry.npmjs.org/nodecg/latest")
    .then((res) => res.json())
    .then((body) => {
    if (semver_1.default.gt(body.version, pjson_1.pjson.version)) {
        console.warn("An update is available for NodeCG: %s (current: %s)", JSON.parse(body).version, pjson_1.pjson.version);
    }
})
    .catch(
