...
var package_default = {
  ...
  scripts: {
    ...
    "dev:server": "node --watch --watch-preserve-output index.js",
    ...
    build: 'del-cli "generated-types/{client,server,shared,types}" dist out && run-s build:{tsc,client,copy-templates,cli,types}',
    ...
    "build:types": "tsx scripts/generate-types-package.ts",
...
// src/cli/commands/defaultconfig.ts
...
// src/cli/lib/install-bundle-deps.ts
...
// src/cli/commands/schema-types.ts
...
function startCommand(program) {
  program.command("start").option("-d, --disable-source-maps", "Disable source map support").description("Start NodeCG").action(async (options) => {
    const cwd = process.cwd();
    if (pathContainsNodeCG(cwd)) {
      if (options.disableSourceMaps) {
        await spawn5("node", ["index.js"], { stdio: "inherit" });
      } else {
        await spawn5("node", ["--enable-source-maps", "index.js"], {
          stdio: "inherit"
        });
      }
      return;
    }
    const nodecgDependencyPath = path6.join(cwd, "node_modules/nodecg");
    if (pathContainsNodeCG(nodecgDependencyPath)) {
      if (options.disableSourceMaps) {
        await spawn5("node", ["node_modules/nodecg/index.js"], {
          stdio: "inherit"
        });
      } else {
        await spawn5(
